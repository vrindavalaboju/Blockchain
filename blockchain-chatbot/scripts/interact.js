async function main() {
    ...
    // Call a function on the contract (assuming you have an owner function)
    const owner = await accessControl.owner();
    console.log("Contract owner:", owner);
    
    // Try validating a query (if you have the validateQuery function implemented)
    try {
      const tx = await accessControl.validateQuery("What is the weather today?");
      const receipt = await tx.wait();
      console.log("Transaction successful:", receipt.transactionHash);
      
      // Look for events (if you have defined events in your contract)
